...
-(BOOL)deleteRowWithId:(int)idRow from:(NSString *)table{
    ...
    if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
        NSString *sql = [NSString stringWithFormat:@"DELETE FROM %@ WHERE empid = %d",table,idRow];
        const char *sqlStatement = (const char*)[sql UTF8String];
        sqlite3_stmt *compiledStatement;
        if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) != SQLITE_OK) {
            NSAssert1(0, @"Error sqlite3_prepare_v2 :. '%s'", sqlite3_errmsg(database));
...
-(BOOL)save:(NSMutableDictionary *)data into:(NSString *)table{
    ...
    if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
        ...
        NSString * sql;
        if ([data valueForKey:@"empid"] == nil) {
            int idUpdate = [[data valueForKey:@"empid"] intValue];
            NSString *updateSql = @"";
            ...
            sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE empid=%d",table, updateSql, idUpdate];
            [data removeObjectForKey:@"empid"];
